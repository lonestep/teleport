...
namespace TLP
{
    ...
    typedef UCHAR           T_BOOL;
    typedef SHORT           T_SHORT;
    typedef PSHORT          T_PSHORT;
    typedef UINT32          T_UINT32;
    typedef INT32           T_INT32;
    typedef PINT32          T_PINT32;
    typedef UINT64          T_UINT64;
    typedef PUINT32         T_PUINT32;
    typedef PUINT64         T_PUINT64;
    typedef DWORD           T_ULONG;
    typedef LONG            T_LONG;
...
#define I64_FMT         "%I64d"
#define T_INVHDL        INVALID_HANDLE_VALUE
...
#define MAX_ID              UINT64_MAX
    ...
    {
        AM_READ = PAGE_READONLY,
        AM_READWRITE = PAGE_READWRITE
...
#define CH_SEND   (1<<1)
#define CH_CREATE_IF_NOEXIST (1<<2)
#define CH_LISTEN_SEND (CH_LISTEN|CH_SEND)
#define CH_ALL (CH_LISTEN_SEND|CH_CREATE_IF_NOEXIST)


    //
    enum class OpenFlag
    {
        LISTEN = CH_LISTEN,                            //Open channel for listening
        SEND = CH_SEND,                                //Open channel for sending
        LISTEN_SEND = CH_LISTEN_SEND,                  //Listen and send, failed if not exist
        CREATE_IF_NOEXIST = CH_CREATE_IF_NOEXIST,      //Create the channel if it's not exist
    ...
    //
    typedef struct _Channel_Record
    {
        T_ID     ChannelId;
        T_CHAR   Guid[MAX_GUID];
        T_USHORT RefCnt;
        T_BOOL   IsGlobal;
    }TChannelRecord, * TPChannelRecord;


    //
    typedef std::vector<T_ID>                     T_ID_VECTOR;
    typedef std::map<T_ID, TPAckRecord>             T_ID_MAP;
    typedef std::map<T_UINT64, TPSessionData>     T_SESSION_MAP;
    ...
    }TPubMessage, * PTPubMessage;


    //
    typedef struct _TeleportConfig
    {
        T_UINT32 nMaxProcPerNamedObject;

    }T_CONFIG, * T_PCONFIG;


    //
    typedef RC(*TLP_CALLBACK)(PTCbMessage pMessage);


    //
    typedef RC(*PFN_ThreadRoutine)(T_PVOID);
...
std::cout.width(20);                                                                        \
if(_x)                                                                                        \
    std::cout<<__FUNCTION__<<"\t\t"<<__FILE__<<"("<<__LINE__<<")"<<"\t[PASS]"<<std::endl;   \
else                                                                                        \
{                                                                                            \
    std::cout<<__FUNCTION__<<"\t\t"<<__FILE__<<"("<<__LINE__<<")"<<"\t[FAILED]"<<std::endl;    \
    exit(-1);                                                                                \
}                                                                                            \
}                                                                                            
...
#define BF_DEFAULT_IVEC 0xbefacedecadedead
